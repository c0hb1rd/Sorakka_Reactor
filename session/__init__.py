...
class AuthSession:
    ...
    def auth_session(cls, f, *args, **options):
        ...
        def decorator(obj, params):
            return f(obj, params) \
                if cls.auth_logic(params, *args, **options) \
                else cls.auth_fail_callback(params, *args, **options)
        return decorator

    # 验证逻辑的接口，返回一个布尔值
    @staticmethod
    def auth_logic(params, *args, **options):
        raise NotImplementedError

    # 验证失败的回调接口
    @staticmethod
    def auth_fail_callback(params, *args, **options):
        raise NotImplementedError
